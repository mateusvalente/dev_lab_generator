  ...
  methods: {
    async load() {
      try {
        ...
        this.message = 'Produtos carregados com sucesso.'
      } catch (error) {
        this.message = error.message || 'Falha ao carregar produtos.'
      }
    },
    ...
    async save() {
      try {
        ...
        this.message = error.message || 'Falha ao salvar produto.'
      }
    },
    async remove(id) {
      try {
        ...
        this.message = error.message || 'Falha ao remover produto.'
      }
    }
  },
  template: `
    <main style="font-family: Arial, sans-serif; padding: 24px; max-width: 1100px; margin: 0 auto;">
      <nav><a href="/index.html">Início</a> | <a href="/categories.html">Categorias</a></nav>
      <h1>Produtos</h1>
